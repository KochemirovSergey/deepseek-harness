...
describe('parseDshArgs', () => {
  it('routes profile boots and the web alias, handing the rest to the app', () => {
    expect(parse(['--profile', 'tui'])).toEqual({ mode: 'profile', profile: 'tui', patches: [], profileReadOnly: false, args: [] })
    expect(parse(['--profile', 'tui', '--patch', 'a.yml', '--patch', 'b.yml']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: ['a.yml', 'b.yml'], profileReadOnly: false, args: [] })
    expect(parse(['web'])).toEqual({ mode: 'profile', profile: 'web', patches: [], profileReadOnly: false, args: [] })
    expect(parse(['web', '--patch', 'web.yml']))
      .toEqual({ mode: 'profile', profile: 'web', patches: ['web.yml'], profileReadOnly: false, args: [] })
    expect(parse(['web', '--profile-read-only']))
      .toEqual({ mode: 'profile', profile: 'web', patches: [], profileReadOnly: true, args: [] })
  })
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['--profile', 'tui', '--resume', 'abc']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: [], profileReadOnly: false, args: ['--resume', 'abc'] })
    expect(parse(['--profile', 'web', '-h']))
      .toEqual({ mode: 'profile', profile: 'web', patches: [], profileReadOnly: false, args: ['-h'] })
    expect(parse(['web', '--host', '127.0.0.1', '--port', '8080', '--no-open', '--future-web-flag']))
      .toEqual({ mode: 'profile', profile: 'web', patches: [], profileReadOnly: false, args: ['--host', '127.0.0.1', '--port', '8080', '--no-open', '--future-web-flag'] })
    expect(parse(['--profile', 'headless', 'run', 'the', 'tests']))
      .toEqual({ mode: 'profile', profile: 'headless', patches: [], profileReadOnly: false, args: ['run', 'the', 'tests'] })
    ...
    expect(parse(['--profile', 'tui', '--patch', 'a.yml', '--resume', 'b', '--patch', 'late.yml']))
      .toEqual({ mode: 'profile', profile: 'tui', patches: ['a.yml'], profileReadOnly: false, args: ['--resume', 'b', '--patch', 'late.yml'] })
  })
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['--profile', 'x', '--profile-read-only', '--dump-config'])).toBe(1)
