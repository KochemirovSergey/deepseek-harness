...
启用管理员所有的[受限实例策略](../../util/launch-environment/README.zh.md)时，通用文件原件仍保存在受保护状态中。`prepareFile` 校验标识、摘要及大小，然后向受约束子进程传输字节，由其在工作区 `.dsh-attachments` 下发布工作副本。模型请求只公开副本路径，不接受任意状态路径。编辑只影响副本，后续请求可能恢复它；需要保留的修改应保存到工作区其他路径。此模式必须提供 sandbox 服务。
