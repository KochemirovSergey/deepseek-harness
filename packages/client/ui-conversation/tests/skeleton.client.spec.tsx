...
it('keeps the restricted composer usable without a selectable workspace', () => {
  const b = mount(sessionSnapshotOf({blank:true}), [], undefined, {summaryBlank:true,restricted:true});
  expect(b.view.getByRole('textbox').getAttribute('contenteditable')).toBe('true');
  expect(b.view.queryByRole('button', {name: t('hero.chooseWorkspace')})).toBeNull();
});
