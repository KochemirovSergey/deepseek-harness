...
Dedicated channels registered with `ctx.connection.rpc.handle()` require an active `webServer`; they fail immediately without one. The caller needs `connection` injection, while Connection resolves the optional Web server through `ctx.get()`. The route is removed when its caller plugin unloads. Shared `/api` registries remain usable without a Web server.
...
A root-owned restricted policy may set `publicOrigin` to one canonical HTTPS origin. A trusted `GET /` without a token bootstraps a signed cookie automatically; API and WebSocket requests still require that cookie and pass the restricted operation policy. Public Host and Origin match exactly, including the HTTPS scheme; forwarded headers cannot establish trust. An external top-level navigation may bootstrap the index directly, without a redirect; cross-site API calls remain denied. Loopback keeps token-based entry. Maintenance never bootstraps anonymous cookies. See the [public-entry decision](../../../.agents/notes/implemented/feature/2026-09-17-public-restricted-web.md).
...
- **Ordinary loopback cookies omit `Secure`** — use the restricted public-origin mode behind HTTPS for anonymous network access; it does not create separate users or private histories.
