...
describe('administrator-owned instance policy', () => {
  ...
  it.each([
    { mode: 'ordinary' }, { statforms: false }, { statforms: 'true' }, { publicOrigin: 'http://example.org' },
  ...
  it('requires explicit administrator opt-in for the source', () => {
    expect(parseInstancePolicy(fixture()).statforms).toBeUndefined()
    expect(parseInstancePolicy({ ...fixture(), statforms: true }).statforms).toBe(true)
  })
