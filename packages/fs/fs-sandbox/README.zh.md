...
启用管理员所有的[受限实例策略](../../util/launch-environment/README.zh.md)时，所有文件操作都通过固定协议在受 Landlock 约束的子进程中使用标准本地后端执行。路径必须位于固定工作区内；即使路径在校验后被替换，也不能访问受保护状态。此模式必须提供 sandbox 服务，权限不能超过 `workspace-write`。下文的逐会话模式说明适用于普通模式。
