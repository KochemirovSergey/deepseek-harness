...
async function connection(factory: () => Transport, retry: boolean, protocolNegotiation: 'auto' | 'legacy' = 'auto') {
  ...
  const handle = startConnection(ctx, { ...config, protocolNegotiation }, resolveReconnectPolicy({
...
describe('SDK negotiation lifecycle', () => {
  it('connects to a discovery-denying gateway only with explicit legacy mode', async () => {
    ...
    const server = createServer(async (request, response) => {
      ...
      if (rpc.method === 'server/discover') { response.writeHead(403).end(); return }
      if (rpc.id === undefined) { response.writeHead(202).end(); return }
      const result = rpc.method === 'initialize'
        ? { protocolVersion: '2025-11-25', capabilities: { tools: {} }, serverInfo: { name: 'readonly', version: '1' } }
        : { tools: [] }
    ...
    const factory = () => new StreamableHTTPClientTransport(new URL(`http://127.0.0.1:${address.port}/mcp`))
    const auto = await connection(factory, false)
    expect((await auto.ready).error).toBeDefined()
    expect(methods).toEqual(['server/discover'])
    await auto.dispose()
    methods.length = 0
    const legacy = await connection(factory, false, 'legacy')
    expect(await legacy.ready).toEqual({})
